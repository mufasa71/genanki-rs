...
pub enum Error {
    ...
    TemplateFormat(#[from] Tmpl),
    #[error(
        "number of model field ({model_len:?}) does not match number of fields ({card_len:?})"
    )]
    ModelFieldCountMismatch { model_len: usize, card_len: usize },
