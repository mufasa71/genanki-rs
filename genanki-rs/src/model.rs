...
impl Model {
    ...
    pub(super) fn req(&self) -> Result<Vec<(usize, String, Vec<usize>)>> {
        ...
        for (template_ord, template) in self.templates.iter().enumerate() {
            ...
            let required_fields = field_values
                ...
                .collect::<Vec<_>>();
            if required_fields.is_empty() {
                return Err(anyhow!(template.clone()));
            }
    ...
    pub(super) fn to_model_db_entry(&self, timestamp: f64, deck_id: i64) -> Result<ModelDbEntry> {
        let templates: Vec<Tmpl> = self
            .templates
            .iter()
            .enumerate()
            .map(|(i, template)| {
    ...
    #[allow(dead_code)]
    pub(super) fn to_json(&self, timestamp: f64, deck_id: i64) -> Result<String> {
        Ok(
...
"#
    ...
    async fn cloze(pool: Pool<Sqlite>) {
        let mut notes = vec![];
        ...
        let db_file_path = conn_options.get_filename();

        deck.write_to_file(out_file.to_str().unwrap(), &mut conn, db_file_path)
            .await
            .unwrap();
    }
